...
# clean all the punctuation and others
def text_clean(text):
    text = text.lower()
    text = re.sub('<br>', ' ', text)
...
# check if the term is in the certain corpus of topic
...
## parsing data part -> clean_text will be tokenized
url = (
...
#一個名叫restaurant_objects的dictionary
restaurant_objects = {}
#第一層會是餐廳
for res in restaurants:
    ...
    #第二層會是針對該餐廳的文章
    for clean_text in articles['res']:
        #第三層會是針對個別的文章 先比較一篇文章的正負評，比大小算出100 010 001那些，然後累加到___score的矩陣中
        for i, term in enumerate(clean_text):
                service_cnt += determine_amount(clean_text, i, "service", corpus)
                food_cnt += determine_amount(clean_text, i, "food", corpus)
                cp_cnt += determine_amount(clean_text, i, "cp", corpus)
...
# 創建一個restaurant物件把它塞進restaurant_object［res］裡面
    restaurant_objects[res] = restaurant()